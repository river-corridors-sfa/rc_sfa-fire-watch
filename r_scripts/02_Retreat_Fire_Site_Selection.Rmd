...
sites <- read_csv(here("Fires_2024", "Retreat_Fire", "site_selection", "Site_List_8_19_2024.csv"), na = c('-9999', 'N/A')) %>%
  distinct(site, .keep_all = TRUE) %>%
  dplyr::select('site','latitude','longitude') %>% 
  drop_na()

sites <- read_csv(here("Fires_2024", "Retreat_Fire", "site_selection", "initial_search", "initial_investigation_08_19_2024.csv"), na = c('-9999', 'N/A')) %>%
  distinct(Site, .keep_all = TRUE) %>%
  rename(site = Site) %>% 
  dplyr::select('site','latitude','longitude') %>% 
  drop_na()
...
```{r}
sites <- sites %>% 
  filter(site != 12) # For some reason site # 12 does not get its COMID pulled so I am removing it. 
