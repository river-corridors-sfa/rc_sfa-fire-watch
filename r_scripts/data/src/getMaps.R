getMaps <- function(x = sites$site, y = sites$comid){
  ...
  flowlines <- st_read('~/GitHub/rc_sfa-fire-watch/r_scripts/data/site_flowlines.gpkg', quiet = T) %>% filter(comid == y)
  ...
  mapshot(plot, file = paste0('~/GitHub/rc_sfa-fire-watch/r_scripts/data/maps/', x, '.jpg'))
}
